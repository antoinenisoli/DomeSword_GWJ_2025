...
func look_mouse(_delta: float) -> void:
    ...
    var angle = v.angle()
    var r = turret_body.global_rotation

    #lerp angle towards the mouse
    var angle_delta = rotate_speed * _delta
    angle = lerp_angle(r, angle + deg_to_rad(90), 1.0)
    angle = clamp(angle, r - angle_delta, r + angle_delta)
    turret_body.global_rotation = angle

    #limit the rotation
    var limit = deg_to_rad(rot_limit)
    turret_body.global_rotation = clamp(turret_body.global_rotation, -limit, limit)

func rotate_turret(_delta: float) -> void:
    var m = Input.get_axis("move_left", "move_right")
    turret_body.rotation += m * rotate_speed * _delta
    var limit = deg_to_rad(rot_limit)
    turret_body.rotation = clamp(turret_body.rotation, -limit, limit)
